...
const statusItems = [
  { text: 'Open to pharmacovigilance & drug safety roles', icon: '💊' },
  { text: 'Studying adverse drug reaction monitoring', icon: '🔬' },
  { text: 'Available for interviews — reach out!', icon: '📞' },
  { text: 'Revising ICH guidelines & GMP standards', icon: '📋' },
  { text: 'Validation mode: ON', icon: '⚗️' },
  { text: 'Reading pharmaceutical research papers', icon: '📚' },
  { text: 'Fresher — ready to contribute from day one', icon: '🚀' },
  { text: 'Exploring drug safety datasets', icon: '📊' },
  { text: 'Calibrating my HPLC knowledge', icon: '💡' },
  { text: 'Sipping chai & studying pharmacovigilance', icon: '☕' },
];

const NAME_TEXT = 'ARHAM HUSSAIN';
...
export default function FooterSection() {
  ...
  const handleCopyEmail = async () => {
    try {
      await navigator.clipboard.writeText('mrarham469@gmail.com');
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch {
      // Fallback
      const textarea = document.createElement('textarea');
      textarea.value = 'mrarham469@gmail.com';
    ...
    <footer style={{ backgroundColor: '#2A2522', overflow: 'visible' }}>
      ...
      >
        <div className="flex items-center gap-2 text-xs" style={{ color: '#A09A94' }}>
          ...
          <Heart className="w-3.5 h-3.5 fill-current" style={{ color: '#0E8B7D' }} />
        </div>
      ...
      <div className="max-w-[1100px] mx-auto px-4 md:px-6 py-10 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          ...
          <div>
            ...
            <a
              href="mailto:mrarham469@gmail.com"
            ...
            <nav className="space-y-3">
              {footerLinks.map((link) => (
                <button
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.color = '#0E8B7D'; }}
      ...
      >
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.borderColor = '#0E8B7D';
            e.currentTarget.style.color = '#0E8B7D';
          }}
          onMouseLeave={(e) => {
