...
const statusItems = [
  { text: 'Open to Solar O&M & renewable energy roles', icon: '☀️' },
  { text: 'Monitoring 100 MW solar plant daily', icon: '📊' },
  { text: 'Available for interviews — reach out!', icon: '📞' },
  { text: 'Reviewing SAP PM module work orders', icon: '💻' },
  { text: 'SCADA monitoring mode: ON', icon: '📶' },
  { text: 'Analysing plant PR & PLF data', icon: '📈' },
  { text: 'Lead Engineer — 2.7+ years of hands-on O&M', icon: '🚀' },
  { text: 'Optimising inverter yield & plant performance', icon: '⚡' },
  { text: 'Calibrating HV testing instruments', icon: '🔬' },
  { text: 'Sipping chai & reviewing energy forecasts', icon: '☕' },
];

const NAME_TEXT = 'AHMED REHAN';
...
export default function FooterSection() {
  ...
  const handleCopyEmail = async () => {
    try {
      await navigator.clipboard.writeText('ahmedrehanmakandar@gmail.com');
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch {
      // Fallback
      const textarea = document.createElement('textarea');
      textarea.value = 'ahmedrehanmakandar@gmail.com';
    ...
    <footer style={{ backgroundColor: '#2A2522', overflow: 'visible' }}>
      ...
      >
        <div className="flex items-center gap-2 text-xs" style={{ color: '#A09A94' }}>
          ...
          <Heart className="w-3.5 h-3.5 fill-current" style={{ color: '#D97706' }} />
        </div>
      ...
      <div className="max-w-[1100px] mx-auto px-4 md:px-6 py-10 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          ...
          <div>
            ...
            <a
              href="mailto:ahmedrehanmakandar@gmail.com"
